...
2. **Implement completely.** For investigation or review, inspect and report; for requested changes, implement and verify the bounded concern. Make routine decisions autonomously; ask when an unresolved choice would change scope, architecture, security, compatibility, or external behavior beyond existing authorization. Continue independent authorized work while clarification is pending. Keep the change narrow, including inseparable adjacent work and synchronized documentation. Complete when behavior, tests, and owned documentation agree without placeholders or unexplained partial work.
3. **Verify by risk.** Use focused checks while iterating and proportionate checks for non-PR documentation work. Before opening or updating a pull request, satisfy the triggered repository PR-readiness guidance. Distinguish source defects from environmental failures before adapting code, and exhaust safe scoped alternatives. Once affected checks and required gates pass, repeat or broaden verification only for subsequent changes, failures, or a specific unresolved risk. Complete when every affected validation lane passes or each boundary and unverified claim is stated precisely.
...
5. **Handoff or publish deliberately.** Mutate Git history or remote state only when requested; carry existing authorization forward without asking again. Report the outcome, material interfaces and files, exact checks, and residual risk; test counts or coverage alone are not proof. Complete when the requested handoff is accurate and its stated gates are satisfied.
...
When the request, documentation, tests, and implementation disagree, present concrete conflict evidence and resolve authoritative intent before encoding behavior. If an instruction blocks authorized work, cite its file and exact rule, and distinguish the requirement from your interpretation.

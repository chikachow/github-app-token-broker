...
5. `installation-access-token-issuance.ts` maps that single evaluation result before any GitHub request.
...
7. The Token Endpoint maps known failures to stable OAuth errors and sanitizes every otherwise unexpected failure to non-cacheable `500 {"error":"server_error"}` without logging raw tokens.

...
The authoritative configured provider and Permit Statement inventories are the checked-in [`configured-token-exchange-composition.ts`](../workers/github-app-token-broker/src/configured-token-exchange-composition.ts) and [`configured-token-issuance-policy.ts`](../workers/github-app-token-broker/src/policy/configured-token-issuance-policy.ts) source. This implementation reference describes how the compiled composition enforces them without duplicating their concrete contents.
